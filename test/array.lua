...
describe("Array", function()
    ...
    describe(":at()", function()
        it("supports negative indices", function()
            local a = Array:of(10, 20, 30)
            expect(a:at(-1)).to.equal(30)
        end)
    end)
    describe(":indexOf", function()
        it("searches for an element", function()
            local a = Array:of(10, 20, 30)
            expect(a:indexOf(30)).to.equal(3)
            expect(a:indexOf(40)).to.equal(-1)
        end)
    end)
    describe(":push()", function()
        it("inserts elements at the end", function()
            local a = Array:of(10, 20):push(30, 40, 50)
            expect(a).to.deep.equal(Array:of(10, 20, 30, 40, 50))
        end)
    end)
    describe(":pop()", function()
        it("deletes the last element", function()
            local a = Array:of(10, 20)
            expect(a:pop()).to.equal(20)
            expect(a:pop()).to.equal(10)
            expect(a:pop()).to.be.null()
        end)
    end)
    describe(":unshift()", function()
        it("inserts elements at the beginning", function()
            local a = Array:of(10, 20):unshift(30, 40, 50)
            expect(a).to.deep.equal(Array:of(30, 40, 50, 10, 20))
        end)
    end)
    describe(":shift()", function()
        it("deletes the first element", function()
            local a = Array:of(10, 20)
            expect(a:shift()).to.equal(10)
            expect(a:shift()).to.equal(20)
